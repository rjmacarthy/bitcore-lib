# Private Key
Represents a stratis private key and is needed to be able to spend stratis and sign transactions. See the official [Stratis Wiki](https://en.stratis.it/wiki/Private_key) for more information about private keys. A PrivateKey in stratcore is an immutable object that has methods to import and export into a variety of formats including [Wallet Import Format](https://en.stratis.it/wiki/Wallet_import_format).
...
Note: The WIF (Wallet Import Format) includes information about the network and if the associated public key is compressed or uncompressed (thus the same stratis address will be generated by using this format).
